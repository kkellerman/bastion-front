...
func _make(cue: String) -> void:
	var loop: bool = cue in ["wind", "birds", "bunker"]
	...
	for i: int in range(bytes.size() / 2):
		...
		if cue == "wind":
			...
			var grit: float = 0.4 if cue.ends_with("dirt") else 0.12
			value = low * 3.0 * exp(-t * 22) + noise * grit * exp(-t * 18)
			if cue.ends_with("metal"): value += sin(t * 1900) * exp(-t * 18) * 0.12
		...
		value *= minf(t * 3000, 1.0) * minf((length - t) * (5 if loop else 30), 1.0)
