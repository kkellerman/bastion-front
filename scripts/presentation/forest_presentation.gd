...
func _trees() -> void:
	...
	for tree: Node3D in $Forest.get_children():
		...
		var variant: int = 2 if index % 5 == 0 else index % 2
	...
	for z: int in range(-86, 36, 4):
		for side: int in [-1, 1]:
			var stone: MeshInstance3D = P.shape(art, Vector3(side * 26.0, 0.4, z), rock, null)
			stone.scale = Vector3(5.5, rng.randf_range(3.5, 6.0), 6.5)
